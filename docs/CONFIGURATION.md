...
egressctl rollback --config /etc/egress-manager/config.json --ipc-key /etc/egress-manager/ipc.key
```
...
`egressctl rollback` restores the latest eligible committed authenticated snapshot and marks that original transaction `ROLLED_BACK`. It first requires an idle healthy journal, exact current owned runtime, valid snapshot and candidate authentication, and component-specific foreign-state preconditions. A missing eligible snapshot exits with code `6`. Route-engine apply/bypass, sing-box, native interface, writable Xray, HAProxy, nftables NAT, and iptables NAT snapshots created by the authenticated journal format are eligible. Legacy plaintext records are recovery-only. Newer unsupported operation formats are never skipped.
