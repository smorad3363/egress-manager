...
`egressctl rollback` restores the latest eligible committed authenticated route-engine snapshot and marks that original transaction `ROLLED_BACK`. It first requires an idle healthy journal, exact current owned runtime, valid snapshot authentication, complete routing inventory, unchanged management boundaries, and no foreign collision. A missing eligible snapshot exits with code `6`. Route-engine apply and bypass snapshots are currently eligible; later operation types fail closed until their snapshot format gains the same manual precondition checks.
