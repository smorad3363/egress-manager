...
`egressctl rollback` restores the latest eligible committed authenticated snapshot and marks that original transaction `ROLLED_BACK`. It first requires an idle healthy journal, exact current owned runtime, valid snapshot authentication, and component-specific foreign-state preconditions. A missing eligible snapshot exits with code `6`. Route-engine apply/bypass, sing-box, native interface, and writable Xray snapshots are eligible. Newer unsupported operation formats are never skipped.
