...
type Outbound = { id: string; name: string; adapter: "sing-box" | "xray" | "interface"; type: string; server: { host: string; port: number }; capabilities: { tcp: boolean; udp: boolean }; health: Health; enabled: boolean; secret_metadata?: string[] };
...
export function OutboundsManager({ createRequest }: { createRequest: number }) {
  ...
  const counts = useMemo(() => {
    ...
    return {
      enabled: items.filter((item) => item.outbound.enabled).length,
      healthy: items.filter((item) => item.outbound.health.status === "healthy").length,
      degraded: items.filter((item) => ["degraded", "unhealthy"].includes(item.outbound.health.status)).length,
      xray: items.filter((item) => item.outbound.adapter === "xray").length,
    };
  }, [state]);
  ...
  return <section>
    <div className="inventory-heading forward-heading"><Heading /><div className="forward-actions"><Button variant="secondary" onClick={() => setImportOpen(true)}>Import outbound</Button><Button variant="primary" disabled={busy} onClick={() => void preview()}>Review sing-box apply</Button></div></div>
    {error ? <div className="inventory-warning" role="alert"><Icon name="shield" /><span>{error}</span></div> : null}
    <div className="forward-summary"><Card><span>Total</span><strong>{state.items.length}</strong><small>Stored adapters</small></Card><Card><span>Enabled</span><strong>{counts.enabled}</strong><small>Desired state</small></Card><Card><span>Xray relay</span><strong>{counts.xray}</strong><small>Listener-relay targets</small></Card><Card><span>Healthy</span><strong>{counts.healthy}</strong><small>{counts.degraded} need attention</small></Card></div>
    {state.items.length === 0 ? <Card className="forward-card"><StatePanel title="No outbounds" description="Import a supported URI or sing-box JSON configuration. VLESS REALITY/XHTTP links are stored as project-owned Xray relay outbounds. Credentials remain encrypted and never return through the API." action="Import outbound" onAction={() => setImportOpen(true)} /></Card> : <div className="outbound-grid">{state.items.map((item) => <OutboundCard key={item.outbound.id} item={item} busy={busy} onTest={() => void testStored(item)} onToggle={() => void toggle(item)} onClone={() => { setCloneSource(item); setCloneForm({ id: `${item.outbound.id}_copy`, name: `${item.outbound.name} copy` }); }} onDelete={() => remove(item)} />)}</div>}
    <Dialog open={importOpen} onOpenChange={setImportOpen} title="Import outbound" description="Paste one supported URI or a bounded sing-box JSON document. VLESS REALITY/XHTTP links are detected automatically and use the project-owned Xray relay adapter."><form className="dialog__form" onSubmit={(event) => { event.preventDefault(); void saveInput(); }}><label className="field"><span className="field__label">URI or sing-box JSON</span><textarea className="input outbound-input" aria-label="URI or sing-box JSON" required value={input} onChange={(event) => { setInput(event.target.value); setTestResult(null); }} placeholder="vless://… or { &quot;outbounds&quot;: […] }" /><span className="field__hint">Supported sing-box imports include VLESS, Trojan, Shadowsocks, VMess, Hysteria2, TUIC, SOCKS5, and WireGuard. VLESS REALITY + XHTTP uses the Xray relay adapter.</span></label>{testResult ? <HealthStrip health={testResult} /> : null}<div className="dialog__actions"><Button variant="ghost" type="button" onClick={() => setImportOpen(false)}>Cancel</Button><Button variant="secondary" type="button" disabled={busy || !input.trim()} onClick={() => void testInput()}>Test connection</Button><Button variant="primary" type="submit" disabled={busy || !input.trim()}>Save outbound</Button></div></form></Dialog>
...
function Heading() { return <div><p className="eyebrow">MANAGED EGRESS ADAPTERS</p><h2>Outbound connections</h2><p>Encrypted credentials, adapter-aware health checks, and explicit runtime ownership. Xray relay outbounds are applied from Listener relays.</p></div>; }
