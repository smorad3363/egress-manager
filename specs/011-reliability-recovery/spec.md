...
- Connected 30-second fixed probes for required system tools, HAProxy configuration and Runtime API, sing-box configuration and service, native interface lifecycle and WireGuard handshake evidence, exact applied routing runtime, and writable Xray configuration/service state. Recovery status and `egressctl status` expose only typed bounded evidence. Integration checkpoint passed full Go test/vet and race tests for app/reliability/routeengine/Xray/interfaceoutbound/egressctl; network candidates and frontend assets are unchanged.
- Added privileged authenticated `egressctl bypass` with stable exit codes and global locking. It journals authenticated snapshots, removes only exact applied protocol-`242` policy state and `inet egm_egress`, preserves persistent desired/applied state and all foreign/NAT/service state, rolls back ordinary failures, and completes crash-interrupted removal deterministically.
- A strict persistent bypass marker prevents startup reconciliation from silently reactivating interception. Status and dependency health expose the state without secrets; repeated bypass is a verified no-op, route Apply is blocked, and explicit `egressctl recover` resumes the last applied state. Decision: [[../../docs/adr/0004-persistent-emergency-bypass|ADR 0004]].
- Emergency-bypass checkpoint passed full Go test/vet, Windows CLI cross-compilation, focused race tests, and the rebuilt two-run privileged network lab. The lab executes the production bypass executor and verifies dual-stack removal, persistent-state preservation, foreign nftables preservation, and idempotence.
- Added authenticated `recovery.rollback` IPC and privileged `egressctl rollback` with busy, privilege, unavailable-snapshot, and generic-failure exit codes. Committed transactions now permit only an explicit rollback transition, and the journal exposes deterministic newest-first committed lookup.
- Route-engine apply and bypass snapshots are manually eligible only after exact current runtime, snapshot authentication, complete inventory, management-path, and foreign-collision preconditions pass. The original transaction becomes the rollback audit record; failed restore remains unfinished for recovery. Other component formats currently fail closed as ineligible. Decision: [[../../docs/adr/0005-manual-snapshot-rollback|ADR 0005]].
- Extended rollback eligibility to sing-box, native interface outbounds, and writable Xray. Each authenticates both snapshot and candidate, checks exact current files and live runtime before transition, and reuses the existing recovery restore path; Xray also binds the unchanged foreign confdir hash.
- Extended authenticated journaling and manual rollback to HAProxy, nftables NAT, and iptables NAT while retaining plaintext legacy records only for crash recovery. HAProxy requires exact configuration, healthy Runtime API state, and native prior-config validation. NAT compares exact normalized owned state and natively validates rollback before entering `ROLLING_BACK`.
- Authenticated HAProxy/NAT rollback checkpoint passed full Go test/vet, focused race tests, Windows `egressctl` cross-build, and two privileged network-lab runs using freshly compiled probes mounted into the pinned prior lab image. Rebuilding the identical image was unavailable because Docker registry DNS lookup failed; no stale probe binary was used. Frontend assets are unchanged, so the prior frontend gate remains applicable. Real disposable-host reboot and the complete failure-injection matrix remain pending.
- Hardened HAProxy and both NAT recovery adapters against authenticated journal corruption: all required protected fields are opened before transaction-state mutation, so tampering stays unfinished and keeps the daemon degraded instead of becoming a finished failure. Regression tests also prove lost HAProxy/NAT runtime blocks manual rollback before `ROLLING_BACK`.
- Extended the privileged lab with a cold volatile-state-loss checkpoint: it removes the owned TUN, nftables table, and dual-stack policy rules/routes, then reconstructs runtime only from persisted applied files and proves the second reconciliation creates no transaction. The executable failure matrix and remaining real-init reboot procedure are documented in [[../../docs/recovery/FAILURE_INJECTION|Phase 11 Failure Injection]].
